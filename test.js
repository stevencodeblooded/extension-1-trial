...
class SemrushExtension {
  ...
  async monitorExtensionStatus() {
    ...
    try {
      ...
      // If we can't write to storage, extension might be being disabled
      console.warn(
        "⚠️ Cannot update active status - extension may be disabled:",
        error
      );

      // Start 5-second countdown
      console.log("🚨 Starting 5-second grace period before logout...");
      setTimeout(async () => {
        console.log("⏰ Grace period expired - performing logout");
        await this.performImmediateLogout();
      }, 5000);
  ...
  async performImmediateLogout() {
    ...
    try {
      ...
      // Mark shutdown as in progress immediately
      await chrome.storage.local.set({
      ...
      console.log("✅ IMMEDIATE AUTO-LOGOUT COMPLETED SUCCESSFULLY");
